...
from sklearn.metrics import confusion_matrix, f1_score, accuracy_score, balanced_accuracy_score, roc_auc_score, roc_curve
...
#load data
dt = pd.read_excel(r'C:\Users\user\Desktop\1\Modeling\21. SG 혼합물 독성 예측\input/SG_binary_data_with_SMILES.xlsx')
mf_A = pd.read_csv('C:\\Users\\user\\Desktop\\1\\Modeling\\21. SG 혼합물 독성 예측\\input\\SG_mf_A.csv')
mf_B = pd.read_csv('C:\\Users\\user\\Desktop\\1\\Modeling\\21. SG 혼합물 독성 예측\\input\\SG_mf_B.csv')
print(dt.shape)
print(mf_A.shape)

smi_A = list(dt['smi_A'])
smi_B = list(dt['smi_B'])
y_list = (list(dt['toxicity']))
        ...
        for num_epoch, lr in lrlr:
            for use_dropout, drop_rate in drdr:
                ...
                for seed in seeds:
                    ...
                    #Make model
                    model = GCNNet_Classification(max_natoms, 
                                n_conv = n_conv, 
                    ...
                    for epoch in range(num_epoch):
                        ...
                        train_preds = np.concatenate(train_outputs)
                        train_outputs = train_preds.round()
                    ...
                    with torch.no_grad():
                        ...
                        test_bce = np.mean(np.array(test_loss_list))
                        print('test loss : ', test_bce)
                        
                    y_pred = pred.round().cpu()
                    y_true = batch['y']
                    test_targets = np.concatenate(test_targets)
                    test_preds = np.concatenate(test_outputs)
                    test_outputs = test_preds.round()
                                    
                                    
                    cm = confusion_matrix(test_targets, test_outputs)
                    print("Confusion Matrix : \n", cm)
                    tn, fp, fn, tp = cm.ravel()
                    acc = accuracy_score(test_targets, test_outputs)
                    auc = roc_auc_score(test_targets, test_preds)
                    sen = tp/(tp+fn)
                    spe = tn/(fp+tn)
                    F1 = f1_score(test_targets, test_outputs)
                    ...
                    cm = confusion_matrix(train_targets, train_outputs)
                    print("Confusion Matrix : \n", cm)
                    tn, fp, fn, tp = cm.ravel()
                    ...
                    train_sen = tp/(tp+fn)
                    train_spe = tn/(fp+tn)
                    train_F1 = f1_score(train_targets, train_outputs)
                    
                    print("Accuracy: ", train_acc)
                    print('ROC-AUC score: ', train_auc)
                    print('Sensitivity: ', train_sen)
                    print('Specificity: ', train_spe)
                    print("F1 score: ", train_F1)
                
                
                
                    #torch.save(model.state_dict(), '0.7026.pth')
                    ...
                    i_results = [seed, now, n_MLP, batch_size, num_epoch, lr, 
                                ...
                                use_bn, use_mf, use_dropout, drop_rate, weight_decay, concat, train_acc, train_auc, train_sen, train_spe, train_F1,
                                acc, auc, sen, spe, F1]
                    total_results.append(i_results)
                    
                    results = pd.read_excel('C:\\Users\\user\\Desktop\\1\\Modeling\\21. SG 혼합물 독성 예측\\output\\20241105_SG_Classification_results.xlsx')
                    results.loc[results.shape[0]] = i_results
                    results.to_excel('C:\\Users\\user\\Desktop\\1\\Modeling\\21. SG 혼합물 독성 예측\\output\\20241105_SG_Classification_results.xlsx',index=False)
...
import pickle
import os 
os.chdir(r'C:\Users\user\Desktop\1\Modeling\20. 혼합독성 예측 모음\SG\Classification')
if True:
    torch.save(model.state_dict(), 'SG_Classification_weights.pth')
